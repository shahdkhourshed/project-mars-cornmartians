...
const OnionsPage = () => {
  ...
  const onionsData = data.find((model) => model.category === 'onions');
  const details = onionsData.details;

  const onions = useFetchData('onions', "", 'onions');

  if (!onions) {
    return <div>Onions not found</div>;
  }

  const OnionsDetails = () => (
    <div className="text-white p-8">
      <h2 className="text-2xl font-bold mb-4">Onions Details</h2>
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center text-xl text-white">
        <div className="w-full xl:w-1/2 p-4">
          <Model3D key={onionsData.model3d} modelPath={onionsData.model3d} initialScale={40} cameraPosition={[0, 0, 180]} />
        </div>
        <div className="w-full xl:w-1/2 p-4">
          <OnionsDetails />
        </div>
